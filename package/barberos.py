...
def POSTbarberos():
    ...
    if 'bar_salario' in request.json and 'usu_num_doc' in request.json:
        ...
        cursor.execute("INSERT INTO t_barbero (bar_id, bar_salario, bar_usu_id) SELECT %s, %s, usu_id FROM t_usuario LEFT JOIN t_barbero ON usu_num_doc = usu_id WHERE usu_num_doc = %s", (bar_id, bar_salario, usu_num_doc,)) #realiazamos consulta sql
